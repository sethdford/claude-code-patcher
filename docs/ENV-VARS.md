...
> All 402 `process.env.*` references extracted from Claude Code v2.1.63, organized by function.
>
> Last scan: 2026-03-01

## Feature Toggle Variables (CLAUDE*CODE*\*)
...
| `CLAUDE_AUTO_BACKGROUND_TASKS`                         | Auto-start background tasks                       |
...
| `USE_LOCAL_OAUTH`                            | Use local OAuth server             |
| `USE_STAGING_OAUTH`                          | Use staging OAuth server           |
| `LOCAL_BRIDGE`                               | Local WebSocket bridge URL         |
...
## CLAUDE\_\* (non-CODE) Variables (13)
...
- [FEATURE-GATES.md](FEATURE-GATES.md) — Patchable feature gates reference (5 patchable, 29 detection-only)
